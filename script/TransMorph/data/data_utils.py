...
def niiload(fname):
    mri_nii = nib.load(fname)
    return mri_nii.get_fdata()

def get_data_seg_from_nii(path):
    x_path = os.path.join(path, 'aligned_norm.nii.gz')
    x_seg_path = os.path.join(path, 'aligned_seg35.nii.gz')
    x = niiload(x_path)
    x_seg = niiload(x_seg_path)
    return x, x_seg