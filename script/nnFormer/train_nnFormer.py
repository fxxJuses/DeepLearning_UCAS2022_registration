...
def main():
    batch_size = 1
    train_dir = '../DATA_OASIS/Train'
    val_dir = '../DATA_OASIS/val'
    weights = [1, 1, 5]  # loss weights
    img_size = (160, 192, 224)
    save_dir = 'nnFormer_ncc_{}_dsx{}_diffusion_{}/'.format(weights[0], weights[1], weights[2])
    ...
    train_set = datasets.OASISBrainDataset(glob.glob(os.path.join(train_dir, '*')), transforms=train_composed)
    val_set = datasets.OASISBrainValDataset(glob.glob(os.path.join(val_dir, '*')), transforms=val_composed)
